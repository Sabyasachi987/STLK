...
program
  .command("cat-file")
  .description("Read and print a blob from .stlk/objects")
...
function createGitDirectory() {
  fs.mkdirSync(path.join(process.cwd(), ".stlk"), { recursive: true });
  fs.mkdirSync(path.join(process.cwd(), ".stlk", "objects"), { recursive: true });
  fs.mkdirSync(path.join(process.cwd(), ".stlk", "refs"), { recursive: true });

  fs.writeFileSync(path.join(process.cwd(), ".stlk", "HEAD"), "ref: refs/heads/main\n");

  // ✅ Create empty .stlkignore if not exists
  const ignorePath = path.join(process.cwd(), ".stlkignore");
  if (!fs.existsSync(ignorePath)) {
    fs.writeFileSync(ignorePath, "# Add files/folders to ignore\n");
  }

  console.log(chalk.blue("Initialized stlk directory"));
}
...
function readFileBlob() {
  ...
  const objectPath = path.join(process.cwd(), ".stlk", "objects", folder, file);
...
async function hashObject(arguments){
    ...
    await fs.mkdirSync(path.join(__dirname,'.stlk','objects',objectFolder),{recursive:true})
    await fs.writeFileSync(path.join(__dirname,'.stlk','objects',objectFolder,objectFile),blobData)
...
function writeTreeRecursive(treeObj) {
  ...
  const objectPath = path.join(".stlk", "objects", folder, filename);

  if (!fs.existsSync(objectPath)) {
    fs.mkdirSync(path.join(".stlk", "objects", folder), { recursive: true });
...
const compressed = fs.readFileSync(path.join('.stlk', 'objects', folder, file));
...
async function commitTree(treeSha, message, parentShas = []) {
  ...
  commitContent +=
    `author stlk <you@example.com> ${timestamp} +0000\n` +
    `committer stlk <you@example.com> ${timestamp} +0000\n\n` +
    `${message}\n`;
  ...
  await fs.mkdirSync(path.join(".stlk", "objects", folder), { recursive: true });
  await fs.writeFileSync(objectPath, zlib.deflateSync(fullBuffer));

  // Update HEAD ref
  const headFileContent = fs.readFileSync(path.join(".stlk", "HEAD"), "utf-8").trim();
  const headRefPath = headFileContent.split(" ")[1]; // e.g. refs/heads/main
  const fullBranchPath = path.join(".stlk", headRefPath);
...
const tagsDir = path.join(".stlk", "refs", "tags");
...
async function logHistory() {
  const headFile = path.join(process.cwd(), ".stlk", "HEAD");

  if (!fs.existsSync(headFile)) {
    console.log(chalk.yellow("No HEAD file found."));
    return;
  }
  ...
  if (headContent.startsWith("ref:")) {
    const ref = headContent.replace("ref: ", "").trim(); // e.g., refs/heads/main
    branchPath = path.join(process.cwd(), ".stlk", ref);
  } else {
    console.log(chalk.red("❌ Detached HEAD not supported yet."));
    return;
  }

  if (!fs.existsSync(branchPath)) {
    console.log(chalk.yellow("No commits yet on this branch."));
    return;
  }

  let commitSha = fs.readFileSync(branchPath, "utf-8").trim();
  ...
  while (commitSha) {
    ...
    parentLines.forEach(line => console.log(chalk.green(line)));
    if (authorLine) console.log(chalk.green(authorLine));
    if (dateString) console.log(chalk.green(`Date:   ${dateString}`));
    console.log(chalk.green(`\n    ${messageLine}\n`));

    // Set next commit in history
    const nextParent = parentLines[0]; // Follow first parent only for now
    if (nextParent) {
      commitSha = nextParent.split(" ")[1];
    } else {
      break;
    }
  }
...
function addPath(inputPath) {
  ...
  if (inputPath === ".stlkignore" || inputPath.startsWith(".stlk")) return;

  // ❌ If ignored (based on your .stlkignore rules), skip it
...
function addFile(filePath) {
  ...
  const dir = path.join(".stlk", "objects", folder);
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, file), compressed);


  // 5. Stage the file in .stlk/index
  const indexPath = path.join(process.cwd(), ".stlk", "index");
  let index = [];
...
function hasConflictMarkers(filePath) {
  ...
  if (filePath.includes(`node_modules${path.sep}`) || filePath.startsWith("node_modules")) {
    return false;
  }
  ...
  try {
    ...
    // Binary or unreadable file (e.g., images, compiled files)
    return false;
  }
}
...
async function commit(args) {
  ...
  const message = args[messageIndex + 1];
   ...
   const mergeHeadPath = path.join(".stlk", "MERGE_HEAD");
  ...
  const headPath = path.join(process.cwd(), ".stlk", "HEAD");
  const headRef = fs.readFileSync(headPath, "utf-8").trim(); // "ref: refs/heads/main"
  const currentBranch = headRef.split(" ")[1]; // "refs/heads/main"
  const branchPath = path.join(".stlk", currentBranch);

  const parentShas = [];

  // 🧩 First parent = current commit on branch
  if (fs.existsSync(branchPath)) {
    const currentHeadSha = fs.readFileSync(branchPath, "utf-8").trim();
    if (currentHeadSha) {
      parentShas.push(currentHeadSha);
    }
  }

  // 🧩 Second parent = from MERGE_HEAD (if it's a merge commit)
  
  if (fs.existsSync(mergeHeadPath)) {
    const secondParentSha = fs.readFileSync(mergeHeadPath, "utf-8").trim();
    if (secondParentSha) {
      parentShas.push(secondParentSha);
    }
  }
  ...
  if (fs.existsSync(mergeHeadPath)) {
    ...
    console.log(chalk.blue("🧹 Merge state cleaned up (MERGE_HEAD removed)."));
  }
}
...
// Read .stlk/index if exists 
function readIndex() {                                      //STATUS HELPER
  const indexPath = path.join(".stlk", "index");
  if (!fs.existsSync(indexPath)) return [];
  return JSON.parse(fs.readFileSync(indexPath, "utf-8"));
}

// Recursively get all file paths (excluding .stlk/)
function getAllFiles(dirPath = ".", fileList = []) {
  ...
  for (const entry of entries) {
    ...
    // Skip .stlk folder and .stlkignore file
    if (fullPath.startsWith(".stlk") || fullPath === ".stlkignore") continue;
...
function createBranch(branchName) {
  const headContent = fs.readFileSync(path.join(".stlk", "HEAD"), "utf8").trim();
  ...
  const currentBranchPath = path.join(".stlk", currentRefPath);
  ...
  const newBranchPath = path.join(".stlk", "refs", "heads", branchName);
...
function listBranches() {
  const headsPath = path.join(".stlk", "refs", "heads");
...
function checkoutBranch(branchName) {
  const branchRef = path.join(".stlk", "refs", "heads", branchName);
  ...
  fs.writeFileSync(path.join(".stlk", "HEAD"), `ref: refs/heads/${branchName}\n`);
...
function readGitObject(sha) {
  ...
  const file = sha.slice(2);
  const objectPath = path.join(".stlk", "objects", dir, file);
...
async function mergeBranch(branchName) {
  const headContent = fs.readFileSync(path.join(".stlk", "HEAD"), "utf-8").trim();
  const headRef = headContent.replace("ref: ", "");
  const currentBranch = path.basename(headRef);
  const currentRefPath = path.join(".stlk", headRef);
  const targetRefPath = path.join(".stlk", "refs", "heads", branchName);
...
async function mergeContinue() {
  ...
  // Prepare to make merge commit
  const mergeHeadPath = path.join(".stlk", "MERGE_HEAD");
  if (!fs.existsSync(mergeHeadPath)) {
    console.error(chalk.red("❌ No merge in progress. MERGE_HEAD not found."));
    return;
  }
  ...
  const headRefPath = path.join(".stlk", "HEAD");
  const headRef = fs.readFileSync(headRefPath, "utf-8").trim().replace("ref: ", "");
  const currentRefPath = path.join(".stlk", headRef);
  const firstParentSha = fs.readFileSync(currentRefPath, "utf-8").trim();

  // Write tree from staged changes
  const treeSha = await writeTreeFromWorkingDirectory(); // or your tree builder

  // Create merge commit with two parents
  const mergeCommitSha = await commitTree(treeSha, "Merge resolved", [firstParentSha, secondParentSha]);

  // Update current branch to point to new commit
  fs.writeFileSync(currentRefPath, mergeCommitSha);

  // Restore working dir (optional)
  restoreFromCommit(mergeCommitSha);

  // Clean up
  fs.unlinkSync(mergeHeadPath);

  console.log(chalk.green(`✅ Merge commit created: ${mergeCommitSha}`));
  console.log(chalk.blue("🧹 Merge state cleaned up."));
}

 
//helper merge continue
function hashBlob(content) {
  ...
  if (!fs.existsSync(objectPath)) {
    fs.mkdirSync(path.join(".stlk", "objects", folder), { recursive: true });
    fs.writeFileSync(objectPath, zlib.deflateSync(Buffer.from(store)));
  }

  return sha;
}
//mergecontinue helper
async function writeTreeFromWorkingDirectory() {
  const allFiles = getAllFiles(".");
  const index = [];

  for (const file of allFiles) {
    if (file.startsWith(".stlk")) continue; // skip internal files
    const content = fs.readFileSync(file);
    const sha = hashBlob(content);
    index.push({ path: file, sha });
  }

   // or move this near your existing writeTree function
...
function currentBranch() {
    try{
          const headPath = path.join(".stlk", "HEAD");
...
async function diffBranches(branchA, branchB) {
  const refA = path.join(".stlk", "refs", "heads", branchA);
  const refB = path.join(".stlk", "refs", "heads", branchB);
